...
function useOutsideClick(containers, cb, features = 1 /* None */) {
  let called = (0, import_react9.useRef)(false);
...
function useFlags(initialFlags = 0) {
  ...
  let addFlag = useEvent((flag) => setFlags((flags2) => flags2 | flag));
  let hasFlag = useEvent((flag) => Boolean(flags & flag));
  let removeFlag = useEvent((flag) => setFlags((flags2) => flags2 & ~flag));
  let toggleFlag = useEvent((flag) => setFlags((flags2) => flags2 ^ flag));
